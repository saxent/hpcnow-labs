...
In this hands-on, we are going to explore potential tuning opportunities in single tasks by using Intel advisor for vectorization and OpenMP parallelisation.
...
When the loop is not vectorized, it reports the reason. Writing the loops in very specific way helps the compiler to identify those opportunities. Otherwise you can define custom SIMD instructions in the code.
...
This generates a new report file (```serial_mm.optrpt```) with the attempts to parallelize the code with OpenMP.
